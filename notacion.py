...
class Matrix:
    ...
    def __pow__(self,n):
        ...
        def MatrixInversa( self ):
            ...
            L = EGJCN(self)
            if L.rank < L.n:  raise ArithmeticError('Matrix singular')
            return Matrix( I(L.n) & T(L.pasos[1]) )
...
class RGC(Matrix):
    def __init__(self, data, rep=0):
        ...
        S       = 'Normal'
        elim    = lambda x: x > p
        def BuscaNuevoPivote(self, r=0, Sentido='Normal'):
            ...
            p = ppivote(self, r, Sentido)
            while p in columnaOcupada:
                p = ppivote(self, p, Sentido)
            return p
        A = Matrix(data);  r = 0;  transformaciones = [];  columnaOcupada = set()
        sentido = lambda x: x if S=='Normal' else reversed(x)
        for i in sentido(range(1,A.m+1)):
            p = BuscaNuevoPivote(i|A , Sentido=S); 
            if p:
                r += 1
                Tr = T([(-Fraction(i|A|j, i|A|p), p, j) for j in filter(elim,range(1,A.n+1))])
...
class RGCsd(Matrix):
    def __init__(self, data, rep=0):
        """Reduce una Matrix con eliminación por columnas (sin divisiones)"""
        S       = 'Normal'
        elim    = lambda x: x > p
        def BuscaNuevoPivote(self, r=0, Sentido='Normal'):
            def ppivote(v, k=0, sentido='Normal'):
            ...
            if p:
                ...
                Tr = T( [ T( [ ( Fraction((i|A|j),(i|A|p)).denominator,   j) ,    \
                               (-Fraction((i|A|j),(i|A|p)).numerator,  p, j)  ] ) \
                                              for j in filter(elim,range(1,A.n+1)) ] )                                 
                transformaciones += [Tr]  if Tr.t else []
                A & T( Tr )
                columnaOcupada.add(p)
        pasos = [[], transformaciones]
...
class RGCN(Matrix):
    def __init__(self, data, rep=0):
        """Reduce una Matrix con eliminación por columnas (pivotes normalizados)"""
        S       = 'Normal'
        elim    = lambda x: x > p
        def BuscaNuevoPivote(self, r=0, Sentido='Normal'):
            def ppivote(v, k=0, sentido='Normal'):
...
class EGC(Matrix):
    def __init__(self, data, rep=0):
        ...
        A = Matrix(data);  r = 0;  transformaciones = [];  columnaOcupada = set()
        sentido = lambda x: x if S=='Normal' else reversed(x)
        pc      = lambda x: x if S=='Normal' else (A.n+1)-x
        for i in sentido(range(1,A.m+1)):
            p = BuscaNuevoPivote(i|A , Sentido=S); 
            if p:
                r += 1
...
class EGCsd(Matrix):
    def __init__(self, data, rep=0):
        """Escalona una Matrix con eliminación por columnas (sin dividir)"""
        S       = 'Normal'
        elim    = lambda x: x > p
        def BuscaNuevoPivote(self, r=0, Sentido='Normal'):
            def ppivote(v, k=0, sentido='Normal'):
...
class EGCN(Matrix):
    def __init__(self, data, rep=0):
        """Escalona una Matrix con eliminación por columnas (pivotes normalizados)"""
        S       = 'Normal'
        elim    = lambda x: x > p
        def BuscaNuevoPivote(self, r=0, Sentido='Normal'):
            def ppivote(v, k=0, sentido='Normal'):
...
class RGF(Matrix):
    def __init__(self, data, rep=0):
        """Reduce una Matrix con eliminación por filas (transf. Gauss)"""
        A = RGC(~Matrix(data))

class RGFsd(Matrix):
    def __init__(self, data, rep=0):
        """Reduce una Matrix con eliminación por filas (sin dividir)"""
        A = RGCsd(~Matrix(data))
        r = A.rank
        pasos = [ list(reversed(A.pasos[1])) , [] ]
        def PasosYEscritura(data,pasos,TexPasosPrev=[]):
            """Escribe en LaTeX los pasos efectivos dados"""
            A   = Matrix(data);  p   = [[],[]]
            tex = latex(data) if len(TexPasosPrev)==0 else TexPasosPrev
...
class RGFN(Matrix):
    def __init__(self, data, rep=0):
        """Reduce una Matrix con eliminación por filas (pivotes normalizados)"""
        A = RGCN(~Matrix(data))
        r = A.rank
        pasos = [ list(reversed(A.pasos[1])) , [] ]
        def PasosYEscritura(data,pasos,TexPasosPrev=[]):
...
class EGFsd(Matrix):
    def __init__(self, data, rep=0):
        """Escalona una Matrix con eliminación por filas (sin dividir)"""
        A = EGCsd(~Matrix(data))
        r = A.rank
        pasos = [ list(reversed(A.pasos[1])) , [] ]
        def PasosYEscritura(data,pasos,TexPasosPrev=[]):
...
class EGFN(Matrix):
    def __init__(self, data, rep=0):
        """Escalona una Matrix con eliminación por filas (pivotes normalizados)"""
        A = EGCN(~Matrix(data))
        r = A.rank
        pasos = [ list(reversed(A.pasos[1])) , [] ]
        def PasosYEscritura(data,pasos,TexPasosPrev=[]):
...
class SubEspacio:
    ...
    def contenidoEn(self, other):
        ...
        if isinstance(other, SubEspacio):
            ...
            return True if (EGCsd(M).rank == other.dim) else False
        else:
...
class SEL:
    def __init__(self, A, b, rep=0):
        ...
        LA = RGCsd ( Matrix(MA) & T(L.pasos[1]) )
        
        if not (LA|0).esNulo():
            BM       = Matrix(BlockMatrix([[MA],[I(MA.n)]]))
            self.tex = tex( {A.m,A.m+A.n}|BM|{A.n} , [[], L.pasos[1]+LA.pasos[1]] )
            raise ArithmeticError('No hay solución: Sistema incompatible')
        
        EA               = I(MA.n) & T(L.pasos[1]) & T(LA.pasos[1])
        Normalizacion    = T([(Fraction(1, 0|EA|0 ), MA.n)])
        EA               = EA & Normalizacion
        self.solP        = list(range(1,A.n+1))|EA|0 

        self.pasos = [ [], L.pasos[1] + LA.pasos[1] + [Normalizacion] ]
        BM         = Matrix(BlockMatrix([ [MA],[I(MA.n)] ]))
...
class SELM:
    def __init__(self, A, b, rep=0):
        ...
        MA = Matrix( BlockMatrix([ [A, -Matrix([b])] ]) )
        LA = RGCsd ( Matrix(MA) )
        #LA = RGCsd ( Matrix(BlockMatrix([[MA],[I(MA.n)]]) ) )

        if not (LA|0).esNulo():
            BM       = Matrix(BlockMatrix([[MA],[I(MA.n)]]))
            self.tex = tex( {A.m,A.m+A.n}|BM|{A.n} , [[], LA.pasos[1]] )
            raise ArithmeticError('No hay solución: Sistema incompatible')
            
        EA    = I(MA.n) & T(LA.pasos[1])
        E     = list(range(1,A.n+1))|EA
        base  = [Vector(E|j) for j in range(1, E.n+1) if Vector(LA|j).esNulo()]
        dim   = len(base)
        self.sgen        = Sistema(base) if dim else Sistema([V0(A.n)])
        self.determinado = (dim == 0)
        ...
        self.pasos = [ [], LA.pasos[1] + [Normalizacion] ]
        BM         = Matrix(BlockMatrix([ [MA],[I(MA.n)] ]))
        self.tex   = tex({A.m,A.m+A.n}|BM|{A.n}, self.pasos)
        self.eafin = EAfin(self.sgen,self.solP)
...
class SELB:
    def __init__(self, A, b, rep=0):
        ...
        filasE = list(range(1,A.n+1))
        
        MA = Matrix( BlockMatrix([ [A, -Matrix([b])] ]) )
        LA = RGCsd ( Matrix(BlockMatrix([[MA],[I(MA.n)]]) ) )
        if not (filasA|LA|0).esNulo():
            self.tex = tex( {A.m,A.m+A.n}|MA|{A.n} , [[], LA.pasos[1]] )
            raise ArithmeticError('No hay solución: Sistema incompatible')
            
        EA    = I(MA.n) & T(LA.pasos[1])
        E     = filasE|EA|filasE
        base  = [Vector(E|j) for j in range(1, E.n+1) if (filasA|LA|j).esNulo()]
        dim   = len(base)
        self.sgen        = Sistema(base) if dim else Sistema([V0(A.n)])
        self.determinado = (dim == 0)
